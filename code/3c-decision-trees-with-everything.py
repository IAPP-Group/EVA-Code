...
def main(args):
    ...
    for social_name, social_sequences in dataset.items():
        for manip_name in social_sequences.keys():
            if args.use_os_info:
                chosen_classes.append('Android-{}-{}'.format(social_name,
                    manip_name))
                chosen_classes.append('iOS-{}-{}'.format(social_name,
                    manip_name))
            else:
                chosen_classes.append('{}-{}'.format(social_name, manip_name))
            ...
            for manip_name, manip_sequences in social_sequences.items():
                for device_id, device_sequences in manip_sequences.items():
                    if args.use_os_info:
                        ...
                        videos_class = '{}-{}-{}'.format(device_os,
                                social_name, manip_name)
                    else:
                        videos_class = '{}-{}'.format(social_name, manip_name)
